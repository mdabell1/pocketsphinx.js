...
startup(function(event) {
    switch(event.data.command){
        ...
        case 'addKeyword':
            ...
            break;
        case 'addKeywordFile':
            addKeywordFile(event.data.data, event.data.callbackId);
            break;
        case 'start':
...
function addKeywordFile(data, clbId) {
    var output;
    if (recognizer) {
        if (data.length > 0) {
            var id_v = new Module.Integers();
            output = recognizer.addKeywordFile(id_v, data);
            if (output != Module.ReturnType.SUCCESS) post({status: "error", command: "addKeywordFile", code: output});
            else post({id: clbId, data: id_v.get(0), status: "done", command: "addKeywordFile"});
            id_v.delete();
        } else post({status: "error", command: "addKeywordFile", code: "js-data"});

    } else post({status: "error", command: "addKeywordFile", code: "js-no-recognizer"});
}
